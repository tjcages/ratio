...
for architecture in arm64 x86_64; do
 xcrun swiftc -target "$architecture-apple-macos12.0" -module-cache-path "${TMPDIR:-/tmp}/ratio-swift-cache" Sources/main.swift -o "$build_dir/Ratio-$architecture" -framework AppKit -framework CoreGraphics -framework ServiceManagement -F vendor/Sparkle -framework Sparkle -Xlinker -rpath -Xlinker @executable_path/../Frameworks
done
